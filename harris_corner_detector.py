...
def bgr_image_to_rgb_image(bgr_image: np.ndarray) -> np.ndarray:
    ...
    rgb_image = bgr_image[:, :, ::-1] # swap the first and last slices by reverse the last dimension 
    return rgb_image
...
def black_and_white_image_to_tiles(arr: np.ndarray, nrows: int,
    ...
    """
    h, w = arr.shape
    tiles_in_row=h//nrows
    return (arr.reshape(tiles_in_row, nrows, -1, ncols)
            .swapaxes(1,2)
...
def create_grad_x_and_grad_y(
    ...
    zeros = np.zeros((input_image.shape[0], 1))
    # concatenate the original array and the new array horizontally
    pad_X= np.concatenate((zeros,input_image), axis=1)
    Ix = pad_X[:,1:] - pad_X[:,:-1]#diff between cols 
    zeros = np.zeros((1, input_image.shape[1]))
    pad_Y= np.concatenate((zeros,input_image),axis=0)
    Iy = pad_Y[1:,:] - pad_Y[:-1,:]#diff between cols 
    Ix[:,0]=0
    Iy[0,:]=0
    return Ix, Iy
...
def calculate_response_image(input_image: np.ndarray, K: float) -> np.ndarray:
    ...
    Sxy = signal.convolve2d(Ix*Iy, g, mode='same')
    det = np.multiply(Sxx, Syy) - np.square(Sxy)
    trace = Sxx + Syy
    return det - K * np.square(trace)
...
def our_harris_corner_detector(input_image: np.ndarray, K: float,
    ...
    TilesList=black_and_white_image_to_tiles(response_image,25,25)   
    maxTileList=[]
    for tile in TilesList:
        maxTile=np.zeros((25,25))
        maxTile[np.unravel_index(np.argmax(tile),(25,25))]=np.max(tile)
        maxTileList.append(maxTile)
    response_image=image_tiles_to_black_and_white_image(np.array(maxTileList),input_image.shape[0], input_image.shape[1])
    output_image = np.zeros(input_image.shape)
    output_image[response_image > threshold] = 1
    return output_image
