...
def compute_normalized_histogram(image: np.ndarray, state: np.ndarray) -> np.ndarray:
    ...
    x_start = max(state[0] - state[2], 0)
    x_end = min(state[0] + state[2], image.shape[1] - 3)
    y_start = max(state[1] - state[3], 0)
    y_end = min(state[1] + state[3], image.shape[0] - 3)
...
# YOU NEED TO FILL THIS PART WITH CODE:
# ........
    print(np.size(S,0),'np.size(S,0)')
    W=np.zeros(np.size(S,0))
    C=np.zeros(np.size(S,0))
    idx=0 
    for n in range (np.size(S,0)):
        idx+=1
        print(idx,'idx')
        print(image.shape)
        print(S[:,n].shape)
